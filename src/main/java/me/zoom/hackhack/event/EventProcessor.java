...
import me.zoom.hackhack.event.events.PlayerJoinEvent;
import me.zoom.hackhack.event.events.PlayerLeaveEvent;
...
public class EventProcessor {
    ...
    public void onKeyInput(InputEvent.KeyInputEvent event) {
        if (Keyboard.getEventKeyState()) {
            ...
            //Macro
            HackHack.getInstance().macroManager.getMacros().forEach(m -> {
    ...
    @SubscribeEvent
    public void onRenderBlockOverlay(RenderBlockOverlayEvent event){ HackHack.EVENT_BUS.post(event); }
                        ...
                        new Thread(() -> {
                            ...
                            if (name != null) {
                                if (mc.player != null && mc.player.ticksExisted >= 1000)
                                    HackHack.EVENT_BUS.post(new PlayerJoinEvent(name));
                            }
                        }).start();
                    }
                }
                ...
                for (SPacketPlayerListItem.AddPlayerData playerData : packet.getEntries()) {
                    if (playerData.getProfile().getId() != mc.session.getProfile().getId()) {
                        new Thread(() -> {
                            ...
                            if (name != null) {
                                if (mc.player != null && mc.player.ticksExisted >= 1000)
                                    HackHack.EVENT_BUS.post(new PlayerLeaveEvent(name));
                            }
                        }).start();
                    }
                }
